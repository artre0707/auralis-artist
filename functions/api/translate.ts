...
// Cloudflare Pages Functions (TypeScript) — hardened
...
async function callGemini(apiKey: string, prompt: string, timeoutMs = 20000, attempt = 0): Promise<string> {
  ...
  try {
    ...
    let data: any;
    try { data = JSON.parse(raw); } catch { throw new Error(`Invalid JSON from Gemini: ${raw}`); }
    ...
    const text = partsText || outputText;
...
export const onRequest: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    // 긴 텍스트는 자동 분할 호출
    const chunks = splitIntoChunks(body.text, 3000);
    const results: string[] = [];

    for (const chunk of chunks) {
      const prompt = buildPrompt({ ...body, text: chunk, target });
      const translated = await callGemini(env.GEMINI_API_KEY, prompt, 20000);
      results.push(translated);
    }

    const joined = results.join("\n\n").trim();
    return new Response(JSON.stringify({ text: joined, chunks: chunks.length }), {
      headers: JSON_HEADERS, status: 200,
    });
